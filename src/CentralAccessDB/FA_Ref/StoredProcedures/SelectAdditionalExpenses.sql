...
create procedure [FA_Ref].[SelectAdditionalExpenses]
(
...
BEGIN
    ...
    BEGIN TRY
		...
		SELECT AdditionalExpenseID,Name, IsActive,CreatedUserID AS UserID,CreatedDate, ModifiedUserID, ModifiedDate
		FROM [fa_ref].AdditionalExpenses
		WHERE AdditionalExpenseID = 
				CASE WHEN 
					@AdditionalExpenseID=-999 
				THEN 
					AdditionalExpenseID 
				ELSE 
					@AdditionalExpenseID 
