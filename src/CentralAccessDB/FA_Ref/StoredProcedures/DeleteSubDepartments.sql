...
create PROCEDURE [FA_Ref].[DeleteSubDepartments]
( 
	@SubDepartmentID				INT,
...
BEGIN
    ...
    BEGIN TRY
		...
		BEGIN TRANSACTION
			...
			EXEC  [ValidateOnDelete] '[fa_ref].[SubDepartments] ', @SubDepartmentID
			SET @HaveValidation =0

			IF @SkipApproval=0
				EXEC [CheckApprovalsForMasterData] @datetoday,@UserId,@ModuleID,'[fa_ref].SubDepartments',' [fa_ref].[SubDepartments]',@DBName,'D',@SnapObject,1,@SubDepartmentID,@HaveApprovals OUTPUT	,@CustomText

			if @HaveApprovals = 0
			BEGIN

				DELETE 
				FROM	[fa_ref].SubDepartments
				...
				EXEC	InsertAuditMaster	4,'D','SubDepartmentID', @ReferenceId, @SubDepartmentID , @UserId, '192.168.1.1',
											'SubDepartments', @datetoday, @datetoday, 'Test', @SnapObject;
			END 
