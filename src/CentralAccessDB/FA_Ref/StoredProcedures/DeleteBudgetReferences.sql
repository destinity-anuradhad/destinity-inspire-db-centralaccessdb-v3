...
CREATE PROCEDURE [FA_Ref].[DeleteBudgetReferences]
( 
...
BEGIN
    ...
    BEGIN TRY
		...
		BEGIN TRANSACTION
			...
			EXEC  [ValidateOnDelete] '[fa_ref].[BudgetReferences] ', @BudgetReferenceID
			SET @HaveValidation =0

			IF @SkipApproval=0
				EXEC [CheckApprovalsForMasterData] @datetoday,@UserId,@ModuleID,'[fa_ref].BudgetReferences',' [fa_ref].[BudgetReferences]',@DBName,'D',@SnapObject,1,@BudgetReferenceID,@HaveApprovals OUTPUT	,@CustomText
			...
			BEGIN
				...
				WHERE	BudgetReferenceID = @BudgetReferenceID
	 
			
				SET	@datetoday = GETDATE();
				SET	@ReferenceId = CAST(@BudgetReferenceID  AS VARCHAR);

				EXEC	InsertAuditMaster	4,'D','BudgetReferenceID', @ReferenceId, @BudgetReferenceID , @UserId, '192.168.1.1',
											'BudgetReferences', @datetoday, @datetoday, 'Test', @SnapObject;
			END 
